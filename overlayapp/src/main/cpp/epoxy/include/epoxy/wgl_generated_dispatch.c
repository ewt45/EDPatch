...
enum wgl_provider {
    ...
    PROVIDER_WGL_10,
    PROVIDER_WGL_3DL_stereo_control,
    PROVIDER_WGL_AMD_gpu_association,
    PROVIDER_WGL_ARB_buffer_region,
    PROVIDER_WGL_ARB_create_context,
    PROVIDER_WGL_ARB_extensions_string,
    PROVIDER_WGL_ARB_make_current_read,
    PROVIDER_WGL_ARB_pbuffer,
    PROVIDER_WGL_ARB_pixel_format,
    PROVIDER_WGL_ARB_render_texture,
    PROVIDER_WGL_EXT_display_color_table,
    PROVIDER_WGL_EXT_extensions_string,
    PROVIDER_WGL_EXT_make_current_read,
    PROVIDER_WGL_EXT_pbuffer,
    PROVIDER_WGL_EXT_pixel_format,
    PROVIDER_WGL_EXT_swap_control,
    PROVIDER_WGL_I3D_digital_video_control,
    PROVIDER_WGL_I3D_gamma,
    PROVIDER_WGL_I3D_genlock,
    PROVIDER_WGL_I3D_image_buffer,
    PROVIDER_WGL_I3D_swap_frame_lock,
    PROVIDER_WGL_I3D_swap_frame_usage,
    PROVIDER_WGL_NV_DX_interop,
    PROVIDER_WGL_NV_copy_image,
    PROVIDER_WGL_NV_delay_before_swap,
    PROVIDER_WGL_NV_gpu_affinity,
    PROVIDER_WGL_NV_present_video,
    PROVIDER_WGL_NV_swap_group,
    PROVIDER_WGL_NV_vertex_array_range,
    PROVIDER_WGL_NV_video_capture,
    PROVIDER_WGL_NV_video_output,
    PROVIDER_WGL_OML_sync_control,
} PACKED;
ENDPACKED
...
static const uint16_t enum_string_offsets[] = {
    ...
    30, /* WGL_AMD_gpu_association */
    54, /* WGL_ARB_buffer_region */
    76, /* WGL_ARB_create_context */
    99, /* WGL_ARB_extensions_string */
    125, /* WGL_ARB_make_current_read */
    151, /* WGL_ARB_pbuffer */
    167, /* WGL_ARB_pixel_format */
    188, /* WGL_ARB_render_texture */
    211, /* WGL_EXT_display_color_table */
    239, /* WGL_EXT_extensions_string */
    265, /* WGL_EXT_make_current_read */
    291, /* WGL_EXT_pbuffer */
    307, /* WGL_EXT_pixel_format */
    328, /* WGL_EXT_swap_control */
    349, /* WGL_I3D_digital_video_control */
    379, /* WGL_I3D_gamma */
    393, /* WGL_I3D_genlock */
    409, /* WGL_I3D_image_buffer */
    430, /* WGL_I3D_swap_frame_lock */
    454, /* WGL_I3D_swap_frame_usage */
    479, /* WGL_NV_DX_interop */
    497, /* WGL_NV_copy_image */
    515, /* WGL_NV_delay_before_swap */
    540, /* WGL_NV_gpu_affinity */
    560, /* WGL_NV_present_video */
    581, /* WGL_NV_swap_group */
    599, /* WGL_NV_vertex_array_range */
    625, /* WGL_NV_video_capture */
    646, /* WGL_NV_video_output */
    666, /* WGL_OML_sync_control */
};
                                   ...
                                   const uint32_t *entrypoints)
{
    ...
    for (i = 0; providers[i] != wgl_provider_terminator; i++) {
        ...
        case PROVIDER_WGL_10:
            if (true)
                return epoxy_gl_dlsym(entrypoint_strings + entrypoints[i]);
            break;
        case PROVIDER_WGL_3DL_stereo_control:
            if (epoxy_conservative_has_wgl_extension(provider_name))
                return wglGetProcAddress(entrypoint_strings + entrypoints[i]);
            break;
        case PROVIDER_WGL_AMD_gpu_association:
            if (epoxy_conservative_has_wgl_extension(provider_name))
                return wglGetProcAddress(entrypoint_strings + entrypoints[i]);
            break;
        case PROVIDER_WGL_ARB_buffer_region:
            if (epoxy_conservative_has_wgl_extension(provider_name))
                return wglGetProcAddress(entrypoint_strings + entrypoints[i]);
            break;
        case PROVIDER_WGL_ARB_create_context:
            if (epoxy_conservative_has_wgl_extension(provider_name))
                return wglGetProcAddress(entrypoint_strings + entrypoints[i]);
            break;
        case PROVIDER_WGL_ARB_extensions_string:
            if (epoxy_conservative_has_wgl_extension(provider_name))
                return wglGetProcAddress(entrypoint_strings + entrypoints[i]);
            break;
        case PROVIDER_WGL_ARB_make_current_read:
            if (epoxy_conservative_has_wgl_extension(provider_name))
                return wglGetProcAddress(entrypoint_strings + entrypoints[i]);
            break;
        case PROVIDER_WGL_ARB_pbuffer:
            if (epoxy_conservative_has_wgl_extension(provider_name))
                return wglGetProcAddress(entrypoint_strings + entrypoints[i]);
            break;
        case PROVIDER_WGL_ARB_pixel_format:
            if (epoxy_conservative_has_wgl_extension(provider_name))
                return wglGetProcAddress(entrypoint_strings + entrypoints[i]);
            break;
        case PROVIDER_WGL_ARB_render_texture:
            if (epoxy_conservative_has_wgl_extension(provider_name))
                return wglGetProcAddress(entrypoint_strings + entrypoints[i]);
            break;
        case PROVIDER_WGL_EXT_display_color_table:
            if (epoxy_conservative_has_wgl_extension(provider_name))
                return wglGetProcAddress(entrypoint_strings + entrypoints[i]);
            break;
        case PROVIDER_WGL_EXT_extensions_string:
            if (epoxy_conservative_has_wgl_extension(provider_name))
                return wglGetProcAddress(entrypoint_strings + entrypoints[i]);
            break;
        case PROVIDER_WGL_EXT_make_current_read:
            if (epoxy_conservative_has_wgl_extension(provider_name))
                return wglGetProcAddress(entrypoint_strings + entrypoints[i]);
            break;
        case PROVIDER_WGL_EXT_pbuffer:
            if (epoxy_conservative_has_wgl_extension(provider_name))
                return wglGetProcAddress(entrypoint_strings + entrypoints[i]);
            break;
        case PROVIDER_WGL_EXT_pixel_format:
            if (epoxy_conservative_has_wgl_extension(provider_name))
                return wglGetProcAddress(entrypoint_strings + entrypoints[i]);
            break;
        case PROVIDER_WGL_EXT_swap_control:
            if (epoxy_conservative_has_wgl_extension(provider_name))
                return wglGetProcAddress(entrypoint_strings + entrypoints[i]);
            break;
        case PROVIDER_WGL_I3D_digital_video_control:
            if (epoxy_conservative_has_wgl_extension(provider_name))
                return wglGetProcAddress(entrypoint_strings + entrypoints[i]);
            break;
        case PROVIDER_WGL_I3D_gamma:
            if (epoxy_conservative_has_wgl_extension(provider_name))
                return wglGetProcAddress(entrypoint_strings + entrypoints[i]);
            break;
        case PROVIDER_WGL_I3D_genlock:
            if (epoxy_conservative_has_wgl_extension(provider_name))
                return wglGetProcAddress(entrypoint_strings + entrypoints[i]);
            break;
        case PROVIDER_WGL_I3D_image_buffer:
            if (epoxy_conservative_has_wgl_extension(provider_name))
                return wglGetProcAddress(entrypoint_strings + entrypoints[i]);
            break;
        case PROVIDER_WGL_I3D_swap_frame_lock:
            if (epoxy_conservative_has_wgl_extension(provider_name))
                return wglGetProcAddress(entrypoint_strings + entrypoints[i]);
            break;
        case PROVIDER_WGL_I3D_swap_frame_usage:
            if (epoxy_conservative_has_wgl_extension(provider_name))
                return wglGetProcAddress(entrypoint_strings + entrypoints[i]);
            break;
        case PROVIDER_WGL_NV_DX_interop:
            if (epoxy_conservative_has_wgl_extension(provider_name))
                return wglGetProcAddress(entrypoint_strings + entrypoints[i]);
            break;
        case PROVIDER_WGL_NV_copy_image:
            if (epoxy_conservative_has_wgl_extension(provider_name))
                return wglGetProcAddress(entrypoint_strings + entrypoints[i]);
            break;
        case PROVIDER_WGL_NV_delay_before_swap:
            if (epoxy_conservative_has_wgl_extension(provider_name))
                return wglGetProcAddress(entrypoint_strings + entrypoints[i]);
            break;
        case PROVIDER_WGL_NV_gpu_affinity:
            if (epoxy_conservative_has_wgl_extension(provider_name))
                return wglGetProcAddress(entrypoint_strings + entrypoints[i]);
            break;
        case PROVIDER_WGL_NV_present_video:
            if (epoxy_conservative_has_wgl_extension(provider_name))
                return wglGetProcAddress(entrypoint_strings + entrypoints[i]);
            break;
        case PROVIDER_WGL_NV_swap_group:
            if (epoxy_conservative_has_wgl_extension(provider_name))
                return wglGetProcAddress(entrypoint_strings + entrypoints[i]);
            break;
        case PROVIDER_WGL_NV_vertex_array_range:
            if (epoxy_conservative_has_wgl_extension(provider_name))
                return wglGetProcAddress(entrypoint_strings + entrypoints[i]);
            break;
        case PROVIDER_WGL_NV_video_capture:
            if (epoxy_conservative_has_wgl_extension(provider_name))
                return wglGetProcAddress(entrypoint_strings + entrypoints[i]);
            break;
        case PROVIDER_WGL_NV_video_output:
            if (epoxy_conservative_has_wgl_extension(provider_name))
                return wglGetProcAddress(entrypoint_strings + entrypoints[i]);
            break;
        case PROVIDER_WGL_OML_sync_control:
            if (epoxy_conservative_has_wgl_extension(provider_name))
                return wglGetProcAddress(entrypoint_strings + entrypoints[i]);
            break;
...
{
    return wgl_single_resolver(PROVIDER_WGL_NV_vertex_array_range, 0 /* wglAllocateMemoryNV */);
}

static PFNWGLASSOCIATEIMAGEBUFFEREVENTSI3DPROC
epoxy_wglAssociateImageBufferEventsI3D_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_I3D_image_buffer, 20 /* wglAssociateImageBufferEventsI3D */);
}

static PFNWGLBEGINFRAMETRACKINGI3DPROC
epoxy_wglBeginFrameTrackingI3D_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_I3D_swap_frame_usage, 53 /* wglBeginFrameTrackingI3D */);
}

static PFNWGLBINDDISPLAYCOLORTABLEEXTPROC
epoxy_wglBindDisplayColorTableEXT_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_EXT_display_color_table, 78 /* wglBindDisplayColorTableEXT */);
}

static PFNWGLBINDSWAPBARRIERNVPROC
epoxy_wglBindSwapBarrierNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_swap_group, 106 /* wglBindSwapBarrierNV */);
}

static PFNWGLBINDTEXIMAGEARBPROC
epoxy_wglBindTexImageARB_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_ARB_render_texture, 127 /* wglBindTexImageARB */);
}

static PFNWGLBINDVIDEOCAPTUREDEVICENVPROC
epoxy_wglBindVideoCaptureDeviceNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_video_capture, 146 /* wglBindVideoCaptureDeviceNV */);
}

static PFNWGLBINDVIDEODEVICENVPROC
epoxy_wglBindVideoDeviceNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_present_video, 174 /* wglBindVideoDeviceNV */);
}

static PFNWGLBINDVIDEOIMAGENVPROC
epoxy_wglBindVideoImageNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_video_output, 195 /* wglBindVideoImageNV */);
}

static PFNWGLBLITCONTEXTFRAMEBUFFERAMDPROC
epoxy_wglBlitContextFramebufferAMD_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_AMD_gpu_association, 215 /* wglBlitContextFramebufferAMD */);
}

static PFNWGLCHOOSEPIXELFORMATARBPROC
epoxy_wglChoosePixelFormatARB_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_ARB_pixel_format, 244 /* wglChoosePixelFormatARB */);
}

static PFNWGLCHOOSEPIXELFORMATEXTPROC
epoxy_wglChoosePixelFormatEXT_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_EXT_pixel_format, 268 /* wglChoosePixelFormatEXT */);
}

static PFNWGLCOPYCONTEXTPROC
epoxy_wglCopyContext_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_10, 292 /* wglCopyContext */);
}

static PFNWGLCOPYIMAGESUBDATANVPROC
epoxy_wglCopyImageSubDataNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_copy_image, 307 /* wglCopyImageSubDataNV */);
}

static PFNWGLCREATEAFFINITYDCNVPROC
epoxy_wglCreateAffinityDCNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_gpu_affinity, 329 /* wglCreateAffinityDCNV */);
}

static PFNWGLCREATEASSOCIATEDCONTEXTAMDPROC
epoxy_wglCreateAssociatedContextAMD_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_AMD_gpu_association, 351 /* wglCreateAssociatedContextAMD */);
}

static PFNWGLCREATEASSOCIATEDCONTEXTATTRIBSAMDPROC
epoxy_wglCreateAssociatedContextAttribsAMD_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_AMD_gpu_association, 381 /* wglCreateAssociatedContextAttribsAMD */);
}

static PFNWGLCREATEBUFFERREGIONARBPROC
epoxy_wglCreateBufferRegionARB_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_ARB_buffer_region, 418 /* wglCreateBufferRegionARB */);
}

static PFNWGLCREATECONTEXTPROC
epoxy_wglCreateContext_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_10, 443 /* wglCreateContext */);
}

static PFNWGLCREATECONTEXTATTRIBSARBPROC
epoxy_wglCreateContextAttribsARB_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_ARB_create_context, 460 /* wglCreateContextAttribsARB */);
}

static PFNWGLCREATEDISPLAYCOLORTABLEEXTPROC
epoxy_wglCreateDisplayColorTableEXT_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_EXT_display_color_table, 487 /* wglCreateDisplayColorTableEXT */);
}

static PFNWGLCREATEIMAGEBUFFERI3DPROC
epoxy_wglCreateImageBufferI3D_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_I3D_image_buffer, 517 /* wglCreateImageBufferI3D */);
}

static PFNWGLCREATELAYERCONTEXTPROC
epoxy_wglCreateLayerContext_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_10, 541 /* wglCreateLayerContext */);
}

static PFNWGLCREATEPBUFFERARBPROC
epoxy_wglCreatePbufferARB_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_ARB_pbuffer, 563 /* wglCreatePbufferARB */);
}

static PFNWGLCREATEPBUFFEREXTPROC
epoxy_wglCreatePbufferEXT_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_EXT_pbuffer, 583 /* wglCreatePbufferEXT */);
}

static PFNWGLDXCLOSEDEVICENVPROC
epoxy_wglDXCloseDeviceNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_DX_interop, 603 /* wglDXCloseDeviceNV */);
}

static PFNWGLDXLOCKOBJECTSNVPROC
epoxy_wglDXLockObjectsNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_DX_interop, 622 /* wglDXLockObjectsNV */);
}

static PFNWGLDXOBJECTACCESSNVPROC
epoxy_wglDXObjectAccessNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_DX_interop, 641 /* wglDXObjectAccessNV */);
}

static PFNWGLDXOPENDEVICENVPROC
epoxy_wglDXOpenDeviceNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_DX_interop, 661 /* wglDXOpenDeviceNV */);
}

static PFNWGLDXREGISTEROBJECTNVPROC
epoxy_wglDXRegisterObjectNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_DX_interop, 679 /* wglDXRegisterObjectNV */);
}

static PFNWGLDXSETRESOURCESHAREHANDLENVPROC
epoxy_wglDXSetResourceShareHandleNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_DX_interop, 701 /* wglDXSetResourceShareHandleNV */);
}

static PFNWGLDXUNLOCKOBJECTSNVPROC
epoxy_wglDXUnlockObjectsNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_DX_interop, 731 /* wglDXUnlockObjectsNV */);
}

static PFNWGLDXUNREGISTEROBJECTNVPROC
epoxy_wglDXUnregisterObjectNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_DX_interop, 752 /* wglDXUnregisterObjectNV */);
}

static PFNWGLDELAYBEFORESWAPNVPROC
epoxy_wglDelayBeforeSwapNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_delay_before_swap, 776 /* wglDelayBeforeSwapNV */);
}

static PFNWGLDELETEASSOCIATEDCONTEXTAMDPROC
epoxy_wglDeleteAssociatedContextAMD_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_AMD_gpu_association, 797 /* wglDeleteAssociatedContextAMD */);
}

static PFNWGLDELETEBUFFERREGIONARBPROC
epoxy_wglDeleteBufferRegionARB_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_ARB_buffer_region, 827 /* wglDeleteBufferRegionARB */);
}

static PFNWGLDELETECONTEXTPROC
epoxy_wglDeleteContext_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_10, 852 /* wglDeleteContext */);
}

static PFNWGLDELETEDCNVPROC
epoxy_wglDeleteDCNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_gpu_affinity, 869 /* wglDeleteDCNV */);
}

static PFNWGLDESCRIBELAYERPLANEPROC
epoxy_wglDescribeLayerPlane_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_10, 883 /* wglDescribeLayerPlane */);
}

static PFNWGLDESTROYDISPLAYCOLORTABLEEXTPROC
epoxy_wglDestroyDisplayColorTableEXT_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_EXT_display_color_table, 905 /* wglDestroyDisplayColorTableEXT */);
}

static PFNWGLDESTROYIMAGEBUFFERI3DPROC
epoxy_wglDestroyImageBufferI3D_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_I3D_image_buffer, 936 /* wglDestroyImageBufferI3D */);
}

static PFNWGLDESTROYPBUFFERARBPROC
epoxy_wglDestroyPbufferARB_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_ARB_pbuffer, 961 /* wglDestroyPbufferARB */);
}

static PFNWGLDESTROYPBUFFEREXTPROC
epoxy_wglDestroyPbufferEXT_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_EXT_pbuffer, 982 /* wglDestroyPbufferEXT */);
}

static PFNWGLDISABLEFRAMELOCKI3DPROC
epoxy_wglDisableFrameLockI3D_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_I3D_swap_frame_lock, 1003 /* wglDisableFrameLockI3D */);
}

static PFNWGLDISABLEGENLOCKI3DPROC
epoxy_wglDisableGenlockI3D_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_I3D_genlock, 1026 /* wglDisableGenlockI3D */);
}

static PFNWGLENABLEFRAMELOCKI3DPROC
epoxy_wglEnableFrameLockI3D_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_I3D_swap_frame_lock, 1047 /* wglEnableFrameLockI3D */);
}

static PFNWGLENABLEGENLOCKI3DPROC
epoxy_wglEnableGenlockI3D_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_I3D_genlock, 1069 /* wglEnableGenlockI3D */);
}

static PFNWGLENDFRAMETRACKINGI3DPROC
epoxy_wglEndFrameTrackingI3D_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_I3D_swap_frame_usage, 1089 /* wglEndFrameTrackingI3D */);
}

static PFNWGLENUMGPUDEVICESNVPROC
epoxy_wglEnumGpuDevicesNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_gpu_affinity, 1112 /* wglEnumGpuDevicesNV */);
}

static PFNWGLENUMGPUSFROMAFFINITYDCNVPROC
epoxy_wglEnumGpusFromAffinityDCNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_gpu_affinity, 1132 /* wglEnumGpusFromAffinityDCNV */);
}

static PFNWGLENUMGPUSNVPROC
epoxy_wglEnumGpusNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_gpu_affinity, 1160 /* wglEnumGpusNV */);
}

static PFNWGLENUMERATEVIDEOCAPTUREDEVICESNVPROC
epoxy_wglEnumerateVideoCaptureDevicesNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_video_capture, 1174 /* wglEnumerateVideoCaptureDevicesNV */);
}

static PFNWGLENUMERATEVIDEODEVICESNVPROC
epoxy_wglEnumerateVideoDevicesNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_present_video, 1208 /* wglEnumerateVideoDevicesNV */);
}

static PFNWGLFREEMEMORYNVPROC
epoxy_wglFreeMemoryNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_vertex_array_range, 1235 /* wglFreeMemoryNV */);
}

static PFNWGLGENLOCKSAMPLERATEI3DPROC
epoxy_wglGenlockSampleRateI3D_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_I3D_genlock, 1251 /* wglGenlockSampleRateI3D */);
}

static PFNWGLGENLOCKSOURCEDELAYI3DPROC
epoxy_wglGenlockSourceDelayI3D_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_I3D_genlock, 1275 /* wglGenlockSourceDelayI3D */);
}

static PFNWGLGENLOCKSOURCEEDGEI3DPROC
epoxy_wglGenlockSourceEdgeI3D_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_I3D_genlock, 1300 /* wglGenlockSourceEdgeI3D */);
}

static PFNWGLGENLOCKSOURCEI3DPROC
epoxy_wglGenlockSourceI3D_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_I3D_genlock, 1324 /* wglGenlockSourceI3D */);
}

static PFNWGLGETCONTEXTGPUIDAMDPROC
epoxy_wglGetContextGPUIDAMD_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_AMD_gpu_association, 1344 /* wglGetContextGPUIDAMD */);
}

static PFNWGLGETCURRENTASSOCIATEDCONTEXTAMDPROC
epoxy_wglGetCurrentAssociatedContextAMD_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_AMD_gpu_association, 1366 /* wglGetCurrentAssociatedContextAMD */);
}

static PFNWGLGETCURRENTCONTEXTPROC
epoxy_wglGetCurrentContext_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_10, 1400 /* wglGetCurrentContext */);
}

static PFNWGLGETCURRENTDCPROC
epoxy_wglGetCurrentDC_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_10, 1421 /* wglGetCurrentDC */);
}

static PFNWGLGETCURRENTREADDCARBPROC
epoxy_wglGetCurrentReadDCARB_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_ARB_make_current_read, 1437 /* wglGetCurrentReadDCARB */);
}

static PFNWGLGETCURRENTREADDCEXTPROC
epoxy_wglGetCurrentReadDCEXT_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_EXT_make_current_read, 1460 /* wglGetCurrentReadDCEXT */);
}
...
{
    return wgl_single_resolver(PROVIDER_WGL_I3D_digital_video_control, 1508 /* wglGetDigitalVideoParametersI3D */);
}

static PFNWGLGETEXTENSIONSSTRINGARBPROC
epoxy_wglGetExtensionsStringARB_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_ARB_extensions_string, 1540 /* wglGetExtensionsStringARB */);
}

static PFNWGLGETEXTENSIONSSTRINGEXTPROC
epoxy_wglGetExtensionsStringEXT_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_EXT_extensions_string, 1566 /* wglGetExtensionsStringEXT */);
}

static PFNWGLGETFRAMEUSAGEI3DPROC
epoxy_wglGetFrameUsageI3D_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_I3D_swap_frame_usage, 1592 /* wglGetFrameUsageI3D */);
}

static PFNWGLGETGPUIDSAMDPROC
epoxy_wglGetGPUIDsAMD_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_AMD_gpu_association, 1612 /* wglGetGPUIDsAMD */);
}

static PFNWGLGETGPUINFOAMDPROC
epoxy_wglGetGPUInfoAMD_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_AMD_gpu_association, 1628 /* wglGetGPUInfoAMD */);
}

static PFNWGLGETGAMMATABLEI3DPROC
epoxy_wglGetGammaTableI3D_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_I3D_gamma, 1645 /* wglGetGammaTableI3D */);
}

static PFNWGLGETGAMMATABLEPARAMETERSI3DPROC
epoxy_wglGetGammaTableParametersI3D_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_I3D_gamma, 1665 /* wglGetGammaTableParametersI3D */);
}

static PFNWGLGETGENLOCKSAMPLERATEI3DPROC
epoxy_wglGetGenlockSampleRateI3D_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_I3D_genlock, 1695 /* wglGetGenlockSampleRateI3D */);
}

static PFNWGLGETGENLOCKSOURCEDELAYI3DPROC
epoxy_wglGetGenlockSourceDelayI3D_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_I3D_genlock, 1722 /* wglGetGenlockSourceDelayI3D */);
}

static PFNWGLGETGENLOCKSOURCEEDGEI3DPROC
epoxy_wglGetGenlockSourceEdgeI3D_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_I3D_genlock, 1750 /* wglGetGenlockSourceEdgeI3D */);
}

static PFNWGLGETGENLOCKSOURCEI3DPROC
epoxy_wglGetGenlockSourceI3D_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_I3D_genlock, 1777 /* wglGetGenlockSourceI3D */);
}

static PFNWGLGETLAYERPALETTEENTRIESPROC
epoxy_wglGetLayerPaletteEntries_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_10, 1800 /* wglGetLayerPaletteEntries */);
}

static PFNWGLGETMSCRATEOMLPROC
epoxy_wglGetMscRateOML_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_OML_sync_control, 1826 /* wglGetMscRateOML */);
}

static PFNWGLGETPBUFFERDCARBPROC
epoxy_wglGetPbufferDCARB_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_ARB_pbuffer, 1843 /* wglGetPbufferDCARB */);
}

static PFNWGLGETPBUFFERDCEXTPROC
epoxy_wglGetPbufferDCEXT_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_EXT_pbuffer, 1862 /* wglGetPbufferDCEXT */);
}

static PFNWGLGETPIXELFORMATATTRIBFVARBPROC
epoxy_wglGetPixelFormatAttribfvARB_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_ARB_pixel_format, 1881 /* wglGetPixelFormatAttribfvARB */);
}

static PFNWGLGETPIXELFORMATATTRIBFVEXTPROC
epoxy_wglGetPixelFormatAttribfvEXT_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_EXT_pixel_format, 1910 /* wglGetPixelFormatAttribfvEXT */);
}

static PFNWGLGETPIXELFORMATATTRIBIVARBPROC
epoxy_wglGetPixelFormatAttribivARB_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_ARB_pixel_format, 1939 /* wglGetPixelFormatAttribivARB */);
}

static PFNWGLGETPIXELFORMATATTRIBIVEXTPROC
epoxy_wglGetPixelFormatAttribivEXT_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_EXT_pixel_format, 1968 /* wglGetPixelFormatAttribivEXT */);
}

static PFNWGLGETPROCADDRESSPROC
epoxy_wglGetProcAddress_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_10, 1997 /* wglGetProcAddress */);
}

static PFNWGLGETSWAPINTERVALEXTPROC
epoxy_wglGetSwapIntervalEXT_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_EXT_swap_control, 2015 /* wglGetSwapIntervalEXT */);
}

static PFNWGLGETSYNCVALUESOMLPROC
epoxy_wglGetSyncValuesOML_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_OML_sync_control, 2037 /* wglGetSyncValuesOML */);
}

static PFNWGLGETVIDEODEVICENVPROC
epoxy_wglGetVideoDeviceNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_video_output, 2057 /* wglGetVideoDeviceNV */);
}

static PFNWGLGETVIDEOINFONVPROC
epoxy_wglGetVideoInfoNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_video_output, 2077 /* wglGetVideoInfoNV */);
}

static PFNWGLISENABLEDFRAMELOCKI3DPROC
epoxy_wglIsEnabledFrameLockI3D_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_I3D_swap_frame_lock, 2095 /* wglIsEnabledFrameLockI3D */);
}

static PFNWGLISENABLEDGENLOCKI3DPROC
epoxy_wglIsEnabledGenlockI3D_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_I3D_genlock, 2120 /* wglIsEnabledGenlockI3D */);
}

static PFNWGLJOINSWAPGROUPNVPROC
epoxy_wglJoinSwapGroupNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_swap_group, 2143 /* wglJoinSwapGroupNV */);
}

static PFNWGLLOADDISPLAYCOLORTABLEEXTPROC
epoxy_wglLoadDisplayColorTableEXT_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_EXT_display_color_table, 2162 /* wglLoadDisplayColorTableEXT */);
}

static PFNWGLLOCKVIDEOCAPTUREDEVICENVPROC
epoxy_wglLockVideoCaptureDeviceNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_video_capture, 2190 /* wglLockVideoCaptureDeviceNV */);
}

static PFNWGLMAKEASSOCIATEDCONTEXTCURRENTAMDPROC
epoxy_wglMakeAssociatedContextCurrentAMD_unwrapped_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_AMD_gpu_association, 2218 /* wglMakeAssociatedContextCurrentAMD */);
}

static PFNWGLMAKECONTEXTCURRENTARBPROC
epoxy_wglMakeContextCurrentARB_unwrapped_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_ARB_make_current_read, 2253 /* wglMakeContextCurrentARB */);
}

static PFNWGLMAKECONTEXTCURRENTEXTPROC
epoxy_wglMakeContextCurrentEXT_unwrapped_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_EXT_make_current_read, 2278 /* wglMakeContextCurrentEXT */);
}

static PFNWGLMAKECURRENTPROC
epoxy_wglMakeCurrent_unwrapped_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_10, 2303 /* wglMakeCurrent */);
}

static PFNWGLQUERYCURRENTCONTEXTNVPROC
epoxy_wglQueryCurrentContextNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_present_video, 2318 /* wglQueryCurrentContextNV */);
}

static PFNWGLQUERYFRAMECOUNTNVPROC
epoxy_wglQueryFrameCountNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_swap_group, 2343 /* wglQueryFrameCountNV */);
}

static PFNWGLQUERYFRAMELOCKMASTERI3DPROC
epoxy_wglQueryFrameLockMasterI3D_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_I3D_swap_frame_lock, 2364 /* wglQueryFrameLockMasterI3D */);
}

static PFNWGLQUERYFRAMETRACKINGI3DPROC
epoxy_wglQueryFrameTrackingI3D_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_I3D_swap_frame_usage, 2391 /* wglQueryFrameTrackingI3D */);
}

static PFNWGLQUERYGENLOCKMAXSOURCEDELAYI3DPROC
epoxy_wglQueryGenlockMaxSourceDelayI3D_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_I3D_genlock, 2416 /* wglQueryGenlockMaxSourceDelayI3D */);
}

static PFNWGLQUERYMAXSWAPGROUPSNVPROC
epoxy_wglQueryMaxSwapGroupsNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_swap_group, 2449 /* wglQueryMaxSwapGroupsNV */);
}

static PFNWGLQUERYPBUFFERARBPROC
epoxy_wglQueryPbufferARB_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_ARB_pbuffer, 2473 /* wglQueryPbufferARB */);
}

static PFNWGLQUERYPBUFFEREXTPROC
epoxy_wglQueryPbufferEXT_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_EXT_pbuffer, 2492 /* wglQueryPbufferEXT */);
}

static PFNWGLQUERYSWAPGROUPNVPROC
epoxy_wglQuerySwapGroupNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_swap_group, 2511 /* wglQuerySwapGroupNV */);
}

static PFNWGLQUERYVIDEOCAPTUREDEVICENVPROC
epoxy_wglQueryVideoCaptureDeviceNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_video_capture, 2531 /* wglQueryVideoCaptureDeviceNV */);
}

static PFNWGLREALIZELAYERPALETTEPROC
epoxy_wglRealizeLayerPalette_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_10, 2560 /* wglRealizeLayerPalette */);
}

static PFNWGLRELEASEIMAGEBUFFEREVENTSI3DPROC
epoxy_wglReleaseImageBufferEventsI3D_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_I3D_image_buffer, 2583 /* wglReleaseImageBufferEventsI3D */);
}

static PFNWGLRELEASEPBUFFERDCARBPROC
epoxy_wglReleasePbufferDCARB_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_ARB_pbuffer, 2614 /* wglReleasePbufferDCARB */);
}

static PFNWGLRELEASEPBUFFERDCEXTPROC
epoxy_wglReleasePbufferDCEXT_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_EXT_pbuffer, 2637 /* wglReleasePbufferDCEXT */);
}

static PFNWGLRELEASETEXIMAGEARBPROC
epoxy_wglReleaseTexImageARB_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_ARB_render_texture, 2660 /* wglReleaseTexImageARB */);
}

static PFNWGLRELEASEVIDEOCAPTUREDEVICENVPROC
epoxy_wglReleaseVideoCaptureDeviceNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_video_capture, 2682 /* wglReleaseVideoCaptureDeviceNV */);
}

static PFNWGLRELEASEVIDEODEVICENVPROC
epoxy_wglReleaseVideoDeviceNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_video_output, 2713 /* wglReleaseVideoDeviceNV */);
}

static PFNWGLRELEASEVIDEOIMAGENVPROC
epoxy_wglReleaseVideoImageNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_video_output, 2737 /* wglReleaseVideoImageNV */);
}

static PFNWGLRESETFRAMECOUNTNVPROC
epoxy_wglResetFrameCountNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_swap_group, 2760 /* wglResetFrameCountNV */);
}

static PFNWGLRESTOREBUFFERREGIONARBPROC
epoxy_wglRestoreBufferRegionARB_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_ARB_buffer_region, 2781 /* wglRestoreBufferRegionARB */);
}

static PFNWGLSAVEBUFFERREGIONARBPROC
epoxy_wglSaveBufferRegionARB_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_ARB_buffer_region, 2807 /* wglSaveBufferRegionARB */);
}

static PFNWGLSENDPBUFFERTOVIDEONVPROC
epoxy_wglSendPbufferToVideoNV_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_NV_video_output, 2830 /* wglSendPbufferToVideoNV */);
}

static PFNWGLSETDIGITALVIDEOPARAMETERSI3DPROC
epoxy_wglSetDigitalVideoParametersI3D_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_I3D_digital_video_control, 2854 /* wglSetDigitalVideoParametersI3D */);
}

static PFNWGLSETGAMMATABLEI3DPROC
epoxy_wglSetGammaTableI3D_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_I3D_gamma, 2886 /* wglSetGammaTableI3D */);
}

static PFNWGLSETGAMMATABLEPARAMETERSI3DPROC
epoxy_wglSetGammaTableParametersI3D_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_I3D_gamma, 2906 /* wglSetGammaTableParametersI3D */);
}

static PFNWGLSETLAYERPALETTEENTRIESPROC
epoxy_wglSetLayerPaletteEntries_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_10, 2936 /* wglSetLayerPaletteEntries */);
}

static PFNWGLSETPBUFFERATTRIBARBPROC
epoxy_wglSetPbufferAttribARB_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_ARB_render_texture, 2962 /* wglSetPbufferAttribARB */);
}

static PFNWGLSETSTEREOEMITTERSTATE3DLPROC
epoxy_wglSetStereoEmitterState3DL_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_3DL_stereo_control, 2985 /* wglSetStereoEmitterState3DL */);
}

static PFNWGLSHARELISTSPROC
epoxy_wglShareLists_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_10, 3013 /* wglShareLists */);
}

static PFNWGLSWAPBUFFERSMSCOMLPROC
epoxy_wglSwapBuffersMscOML_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_OML_sync_control, 3027 /* wglSwapBuffersMscOML */);
}

static PFNWGLSWAPINTERVALEXTPROC
epoxy_wglSwapIntervalEXT_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_EXT_swap_control, 3048 /* wglSwapIntervalEXT */);
}

static PFNWGLSWAPLAYERBUFFERSPROC
epoxy_wglSwapLayerBuffers_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_10, 3067 /* wglSwapLayerBuffers */);
}

static PFNWGLSWAPLAYERBUFFERSMSCOMLPROC
epoxy_wglSwapLayerBuffersMscOML_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_OML_sync_control, 3087 /* wglSwapLayerBuffersMscOML */);
}

static PFNWGLUSEFONTBITMAPSAPROC
epoxy_wglUseFontBitmapsA_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_10, 3113 /* wglUseFontBitmapsA */);
}

static PFNWGLUSEFONTBITMAPSWPROC
epoxy_wglUseFontBitmapsW_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_10, 3132 /* wglUseFontBitmapsW */);
}

static PFNWGLUSEFONTOUTLINESPROC
epoxy_wglUseFontOutlines_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_10, 3151 /* wglUseFontOutlines */);
}

static PFNWGLUSEFONTOUTLINESAPROC
epoxy_wglUseFontOutlinesA_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_10, 3170 /* wglUseFontOutlinesA */);
}

static PFNWGLUSEFONTOUTLINESWPROC
epoxy_wglUseFontOutlinesW_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_10, 3190 /* wglUseFontOutlinesW */);
}

static PFNWGLWAITFORMSCOMLPROC
epoxy_wglWaitForMscOML_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_OML_sync_control, 3210 /* wglWaitForMscOML */);
}

static PFNWGLWAITFORSBCOMLPROC
epoxy_wglWaitForSbcOML_resolver(void)
{
    return wgl_single_resolver(PROVIDER_WGL_OML_sync_control, 3227 /* wglWaitForSbcOML */);
}
