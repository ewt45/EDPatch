...
import static com.ewt45.exagearsupportv7.input.CHCharSupport.avaiKeyCode;
import static com.ewt45.exagearsupportv7.input.CHCharSupport.currIndex;
...
//        Log.d(TAG, "convertUnicodeToXKey: 接收到的字符对应unicode编码为"+i);
        //判断是否在65536范围内，如果不是的话手动新建一个Xkey返回 (中文标点啥的也不在基础汉字范围内，那还要判断一下是否为null)
        XKey returnKey = null;
        if(i>65536 || this.UnicodeToXKeyMap[i] ==null){
            Log.d(TAG, "convertUnicodeToXKey: 超出数组65535范围");
            returnKey = new XKey(avaiKeyCode[currIndex],i+0x1000000);
            currIndex = (currIndex+1)%avaiKeyCode.length; //数组下标+1，为下一次设置另一个keycode做准备
        }
        //外部无法访问XKey，只能放到这里了
        //判断是否为中文，修改其keycode 判断就用keysym是否大于0x1000000判断了，ex里貌似原本没有这么大的(失误了，非unicode字符也调用这个，有个flag是 COMBINING_ACCENT = 0x80000000
        else if (this.UnicodeToXKeyMap[i].keysym > 0x1000000) {
            this.UnicodeToXKeyMap[i].keycode= avaiKeyCode[currIndex]; //如果是，设置一个keycode防止重复
            currIndex = (currIndex+1)%avaiKeyCode.length; //数组下标+1，为下一次设置另一个keycode做准备
            returnKey = this.UnicodeToXKeyMap[i];
        }
        return returnKey == null? this.UnicodeToXKeyMap[i] :returnKey;
    }

    /** 初始化时填充数组，用于将输入字符转换成xkey格式（应该是）
     * UnicodeToXKeyMap数组下标代表字符的unicode编码（utf-16) */
    private void constructUnicodeToXKeyMap() {
        //添加中文keysym
        Log.d(TAG, "constructUnicodeToXKeyMap: 添加中文字符支持");
        for(int i = 0x4e00; i<=0x9fa5; i++){
            this.UnicodeToXKeyMap[i] = new XKey(KeyCodesX.KEY_H, i+0x1000000);
        }
