...
public class WineHQParser {
    File packageFile = new File(Globals.getAppContext().getFilesDir(), "image/opt/wineCollection/winePackages/official-Packages");

    int pos = 0;
    private final List<WineHQWineInfo> stableList= new ArrayList<>();
    private final List<WineHQWineInfo> develList= new ArrayList<>();
    private final List<WineHQWineInfo> stagingList= new ArrayList<>();
    private final WineHQParser.Callback mCallback;


    public WineHQParser(Callback callback) {
        mCallback = callback;
        refresh();
    ...
    public List<WineHQWineInfo> getListByBranch(WineHQAdapter.Branch branch){
        switch (branch){
            case STABLE:
                return stableList;
            case DEVEL:
    ...
    private void parsePackages() throws IOException {
        ...
        List<WineHQWineInfo> winePackagesInfos = new ArrayList<>();
        pos = 0;

        while (pos < allLines.size()) {
            WineHQWineInfo info = new WineHQWineInfo();
        ...
        //按种类分类
        for (WineHQWineInfo info : winePackagesInfos) {
        ...
        Comparator<WineHQWineInfo> listComparator = new Comparator<WineHQWineInfo>() {
            @Override
            public int compare(WineHQWineInfo o1, WineHQWineInfo o2) {
