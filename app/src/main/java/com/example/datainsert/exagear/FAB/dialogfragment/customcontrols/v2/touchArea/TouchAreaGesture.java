...
import static com.example.datainsert.exagear.FAB.dialogfragment.customcontrols.v2.gestureMachine.FSMR.event.手指距离指针_近;
import static com.example.datainsert.exagear.FAB.dialogfragment.customcontrols.v2.gestureMachine.FSMR.event.手指距离指针_远;
...
import com.example.datainsert.exagear.FAB.dialogfragment.customcontrols.v2.gestureMachine.State.ActionRunOption;
import com.example.datainsert.exagear.FAB.dialogfragment.customcontrols.v2.gestureMachine.State.State1FingerMoveToMouseMove;
import com.example.datainsert.exagear.FAB.dialogfragment.customcontrols.v2.gestureMachine.State.State2FingersZoom;
import com.example.datainsert.exagear.FAB.dialogfragment.customcontrols.v2.gestureMachine.State.StateCheckFingerNearToPointer;
...
import com.example.datainsert.exagear.FAB.dialogfragment.customcontrols.v2.gestureMachine.State.StateCountDownWaitFingerNumChange;
import com.example.datainsert.exagear.FAB.dialogfragment.customcontrols.v2.gestureMachine.State.StateFingerMoveToMouseScroll;
...
//        );
        ...
        //1指点击，鼠标左键点击
        machine.addTransition(stateWatch1F, 某手指_未移动并松开, stateWaitDoubleTapCountDown, actionMouseMoveToFinger, actionLeftClick);
        //1指立刻移动，鼠标滚轮
        machine.addTransition(stateWatch1F, 手指_移动_慢速, stateMouseScroll, actionMouseMoveToFinger);
        machine.addTransition(stateWatch1F, 手指_移动_快速, stateMouseScroll, actionMouseMoveToFinger);

        //1指长按后，等待移动或松手
        machine.addTransition(stateWatch1F, 手指_未移动, stateWait1FActionAfterLongPress);
        //1指长按后松开，右键点击
        machine.addTransition(stateWait1FActionAfterLongPress, 某手指_未移动并松开, stateWaitForNeutral, actionMouseMoveToFinger , actionRightClick);
...
//        machine.addTransition(stateWait1FActionAfterLongPress, StateCountDownMeasureSpeed.SLOW_MOVE, actionPressLeftMouse);//由于不会超时，所以不会产生slow_move事件
        machine.addTransition(stateWait1FActionAfterLongPress, 手指_移动_快速, state1FingerMoveToMouseMove, actionMouseMoveToFinger, actionPressLeftMouse);
        machine.addTransition(state1FingerMoveToMouseMove, 完成, stateWaitForNeutral, stateReleaseLeftMouse);

        //第一次左键单击后，短时间内检查是否有第二次左键单击形成双击，如果有则第二次点击时不移动鼠标位置，防止微小的鼠标移动导致系统不识别双击
        machine.addTransition(stateWaitDoubleTapCountDown, 新手指按下, checkIfNearPointer);
        //如果距离近则跳过移动鼠标位置 直接点击左键，(更精确的话此时应该只是按下左键，等手指松开了才能松开左键，不过直接点击应该也没啥问题吧）
        machine.addTransition(checkIfNearPointer, 手指距离指针_近, stateWaitForNeutral, actionLeftClick);
        //否则仿佛从初始状态开始触摸手指一下，移动指针位置并走到检测1指那一步
        machine.addTransition(checkIfNearPointer, 手指距离指针_远, stateWatch1F, actionMouseMoveToFinger);

        //2指按下，观测
        machine.addTransition(stateWatch1F, 新手指按下, stateWatch2F);
//        machine.addTransition(stateMouseScroll, 新手指按下, stateWatch2F);
        machine.addTransition(stateWait1FActionAfterLongPress, 新手指按下, stateWatch2F);
        //2指不动，观察
        machine.addTransition(stateWatch2F, 手指_未移动, stateWait2FDoSth);

        //2指点击，二指事件（显示键盘）
        machine.addTransition(stateWatch2F, 某手指_未移动并松开, stateWaitForNeutral, actionShowSoftInput);
        machine.addTransition(stateWait2FDoSth, 某手指_未移动并松开, stateWaitForNeutral, actionShowSoftInput);
        //2指移动，缩放
        machine.addTransition(stateWatch2F, 手指_移动_慢速, state2FingersZoom);
        machine.addTransition(stateWatch2F, 手指_移动_快速, state2FingersZoom);
        machine.addTransition(stateWait2FDoSth, 手指_移动_快速, state2FingersZoom);

        //3指按下并松开，或者4指按下
        machine.addTransition(stateWatch2F, 新手指按下, stateWait3FDoSth);
        machine.addTransition(stateWait2FDoSth, 新手指按下, stateWait3FDoSth);

        machine.addTransition(stateWait3FDoSth, 某手指_未移动并松开, stateWaitForNeutral, actionShowAllOptions);
        machine.addTransition(stateWait3FDoSth, 新手指按下, stateWaitForNeutral, actionShowAllOptions);
        //TODO 尚未实现缩放后一指移动。
